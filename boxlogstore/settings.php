...
if ($hassiteconfig) {
    ...
    $menuroutes = [];
    $defaultSelection = [];
    $eventInformation = \src\transformer\get_event_information();
    foreach (array_keys($eventInformation) as $eventname) {
        $menuroutes[$eventname] = $eventname;
        $defaultSelection[$eventname] = $eventInformation[$eventname]['enabled_by_default'];
    }
